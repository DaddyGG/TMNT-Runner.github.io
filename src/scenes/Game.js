...
export default class Game extends Phaser.Scene {
    ...
    create() {
        ...
        this.playerCollider = this.physics.add.collider(this.player, this.bg8, () => {
        ...
        this.physics.add.overlap(this.player, this.bg8, () => {
            ...
            this.player.setPosition(200, this.height - 150);
        });
        ...
        this.physics.add.collider(this.player, this.platformGroup, (s, s1) => {
    ...
    platformOverlap() {
        if (this.platformGroup.getLength()) {
            this.platformGroup.getChildren().forEach(platform => {
                const platformPosY = platform.body.y - platform.body.height;

                this.physics.add.overlap(this.player, platform, () => {

                    this.player.y = platformPosY - (this.player.body.height / 2);
                });
            });
        }
    }
    ...
    spikeOverlap() {
        if (this.spikeGroup.getLength()) {
            this.spikeGroup.getChildren().forEach(spike => {


                const o = this.physics.add.overlap(this.player, spike, () => {
    ...
    backgroundParallax() {
        ...
        const fact = [1.45, 1.5, 1.65, 1.75, 1.85, 2.1, 3.55, 6.8];
